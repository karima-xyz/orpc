import type { AnyContractProcedure, AnyContractRouter, AnySchema, ErrorMap, OpenAPIV3 as OpenAPI } from '@orpc/contract'
...
import { applyCustomOpenAPIOperation } from './openapi-v3-custom'
import { checkParamsSchema, resolveOpenAPIJsonSchemaRef, toOpenAPIContent, toOpenAPIEventIteratorContent, toOpenAPIMethod, toOpenAPIParameters, toOpenAPIPath, toOpenAPISchema } from './openapi-v3-utils'
...
export interface OpenAPIV3GeneratorOptions extends StandardOpenAPIJsonSerializerOptions {
  schemaConverters?: ConditionalSchemaConverter[]
}

export interface OpenAPIV3GeneratorGenerateOptions extends Partial<Omit<OpenAPI.Document, 'openapi'>> {
  /**
 ...
 */
export class OpenAPIV3Generator {
   ...
   * @see {@link https://orpc.unnoq.com/docs/openapi/openapi-specification OpenAPI Specification Docs}
   */
  async generate(router: AnyContractRouter | AnyRouter, options: OpenAPIV3GeneratorGenerateOptions = {}): Promise<OpenAPI.Document> {
    ...
    for (const { contract, path } of contracts) {
      ...
      try {
        ...
        if (def.route.specv3 !== undefined && typeof def.route.specv3 !== 'function') {
          operationObjectRef = def.route.specv3
        }
        else {
          operationObjectRef = {
        ...
        if (typeof def.route.specv3 === 'function') {
          operationObjectRef = def.route.specv3(operationObjectRef)
        }
  ...
  async #resolveCommonSchemas(doc: OpenAPI.Document, commonSchemas: OpenAPIV3GeneratorGenerateOptions['commonSchemas']): Promise<{
